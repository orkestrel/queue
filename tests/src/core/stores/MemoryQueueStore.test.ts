...
//
// A real `input` payload is often a structured object, not a scalar. The memory store
// validates the supplied shape and clones the input into owned JSON, so
// nested fields survive while caller identity does not leak into the backing map.

describe('MemoryQueueStore — structured-object inputs', () => {
	it('round-trips a nested-object input as an immutable owned snapshot', async () => {
		const store = new MemoryQueueStore(
			objectShape({
				url: stringShape(),
				headers: objectShape({ accept: stringShape(), retries: numberShape() }),
			}),
		)
		...
		// The returned graph is frozen and does not share caller identity.
		expect(entry?.input).not.toBe(input)
		expect(Object.isFrozen(entry?.input)).toBe(true)
		expect(Object.isFrozen(entry?.input.headers)).toBe(true)
		// Typed access compiles (the class is generic over the input, not `unknown`).
		expect(entry?.input.headers.accept).toBe('json')
	})
})

describe('MemoryQueueStore contract and ownership', () => {
	it('owns snapshots across caller and load-result mutation attempts', async () => {
		const store = createMemoryQueueStore(
			objectShape({ label: stringShape(), nested: objectShape({ value: stringShape() }) }),
		)
		const input = { label: 'original', nested: { value: 'kept' } }
		await store.save({ id: 'job', input, attempts: 0 })
		input.label = 'changed'
		input.nested.value = 'changed'
		const first = await store.load()
		const second = await store.load()
		expect(first[0]?.input).toEqual({ label: 'original', nested: { value: 'kept' } })
		expect(first[0]?.input).not.toBe(input)
		expect(first[0]?.input).not.toBe(second[0]?.input)
		expect(Object.isFrozen(first[0]?.input.nested)).toBe(true)
	})

	it('enforces the supplied shape at the runtime save boundary', async () => {
		const store = createMemoryQueueStore(stringShape())
		const saving = Reflect.apply(store.save, store, [{ id: 'invalid', input: 42, attempts: 0 }])
		await expect(saving).rejects.toSatisfy(
			(error: unknown) => isQueueError(error) && error.code === 'store',
		)
		expect(await store.load()).toEqual([])
	})

	it('captures a stateful id accessor once and keys the owned snapshot by that value', async () => {
		const store = createMemoryQueueStore(stringShape())
		let reads = 0
		const entry = {
			get id() {
		...
		await expect(Reflect.apply(store.save, store, [entry])).resolves.toBeUndefined()
		expect(reads).toBe(1)
		expect(await store.load()).toEqual([{ id: 'first', input: 'payload', attempts: 0 }])
		await store.remove('first')
		expect(await store.load()).toEqual([])
	})

	it('contains a hostile entry accessor as a coded save failure', async () => {
		const store = createMemoryQueueStore(stringShape())
		const entry = Object.defineProperty({ id: 'hostile', input: 'payload', attempts: 0 }, 'input', {
			get() {
				throw new Error('hostile input getter')
			},
		})
		await expect(Reflect.apply(store.save, store, [entry])).rejects.toSatisfy(
			(error: unknown) =>
				isQueueError(error) && error.code === 'store' && error.context?.operation === 'save',
		)
		expect(await store.load()).toEqual([])
	})
})
